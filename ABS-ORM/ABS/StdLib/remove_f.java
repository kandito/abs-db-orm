        ...
        return (new abs.backend.java.lib.expr.Case() {
            public ABS.StdLib.Set<A> of(final A e, final ABS.StdLib.Set<A> xs, final ABS.StdLib.Set<A> __ABS_value) {
                ...
                if (__ABS_binding2 != null) return new Object() {
                    public ABS.StdLib.Set<A> execute(final A x, final ABS.StdLib.Set<A> ss) { return e.lt(x).toBoolean() ? xs : new ABS.StdLib.Set_Insert(x, ABS.StdLib.remove_f.apply(ss, e)); }
                }.execute((A) __ABS_binding2.getBinding(0),(ABS.StdLib.Set<A>) __ABS_binding2.getBinding(1));
                throw new abs.backend.java.lib.expr.UnmatchedCaseException("abslang.abs:118:3:  value " + __ABS_value + " did not match any pattern.");
            }
        }.of(e, xs, xs));
    }
}
